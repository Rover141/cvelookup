...
args.csv=args.product+"-"+args.version
if args.csv:
    try:
        with open(dir_path+"/result/output.csv", 'a') as f:
            f.write('Product_Version;CVE-ID;CVSS;Date;Description;URL;Exploit?\n')
            for r in results:
                f.write('{inf};{cve};{cvss};{date};{description};{url};{exploit}\n'.format(
                    inf=args.csv,
                    cve=r['cve_id'],
                    cvss=r['cvss_score'],
                    date=r['publish_date'],
